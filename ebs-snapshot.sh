...
snapshot_volumes() {
	for volume_id in $volume_list; do
		...
		snapshot_description="${instance_id}-${instance_name}-$(hostname)-${device_name}-backup-$(date +%Y-%m-%d)"
		...
		aws ec2 create-tags --region $region --resource $snapshot_id --tags Key=CreatedBy,Value=AutomatedBackup Key=Name,Vlaue=${snapshot_description}
	done
}
